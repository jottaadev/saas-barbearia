...
export default function AgendaCompletaPage() {
  ...
  const [filters, setFilters] = useState({ date: new Date(), barberId: '', status: '' });
          ...
          <div className="relative w-full md:w-auto" ref={calendarRef}>
            ...
            {isCalendarOpen && (
              <div className="absolute top-full mt-2 z-10 bg-zinc-800 rounded-md border border-zinc-700 shadow-lg">
                <DayPicker mode="single" selected={filters.date} onSelect={handleDateChange} locale={ptBR} fromDate={new Date(2020, 1, 1)} />
              </div>
            )}
          </div>
            ...
            <tbody>
              {appointments.length > 0 ? appointments.map(app => (
                <tr key={app.id} className="border-t border-zinc-800 hover:bg-zinc-800/50">
                  ...
                  <td className="p-4">
                    <span className={`px-2 py-1 text-xs font-bold rounded-full ${app.status === 'Concluído' ? 'bg-green-500/20 text-green-400' : app.status === 'Cancelado' ? 'bg-red-500/20 text-red-400' : 'bg-blue-500/20 text-blue-400'}`}>
                      {app.status}
                    </span>
                  </td>
                </tr>
              )) : (
                !isLoading && <tr><td colSpan="5" className="p-4 text-center text-zinc-500">Nenhum agendamento encontrado.</td></tr>
              )}
            </tbody>
          </table>
        </div>
