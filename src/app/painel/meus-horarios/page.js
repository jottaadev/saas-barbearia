...
import { Calendar as CalendarIcon, Lock, Unlock, AlertCircle, Loader2, Plane, Trash2 } from 'lucide-react';
...
export default function MeusHorariosPage() {
    ...
    const handleSlotClick = async (slot) => {
        ...
        try {
            const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'https://backend-barber-5sbe.onrender.com';
            if (action === 'block') {
            ...
            toast.error('Ocorreu um erro ao atualizar o horário.');
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
                            ...
                            <div className="flex flex-col gap-4">
                                ...
                                <div className="mt-4 pt-4 border-t border-zinc-800">
                                  <h3 className="font-bold text-white mb-2">Ausências agendadas:</h3>
                                  <div className="space-y-2 max-h-40 overflow-y-auto pr-2">
                                      {isLoading.absences ? <p className="text-zinc-400">A carregar...</p> : absences.length > 0 ? absences.map(abs => (
                                          <div key={abs.id} className="flex justify-between items-center bg-zinc-800 p-3 rounded-md">
                                              <p className="text-sm">{format(new Date(abs.start_date), 'dd/MM/yy')} - {format(new Date(abs.end_date), 'dd/MM/yy')}</p>
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center gap-1 text-xs mt-1">
                                                {isProcessing ? <Loader2 className="animate-spin" size={12}/> : 
                                                 isAvailable ? <><Unlock size={12}/>Disponível</> :
                                                 isBlocked ? <><Lock size={12}/>Bloqueado</> :
                                                 isBooked ? <><CalendarIcon size={12}/>Agendado</> :
                                                 slot.status
                                                }
                                            </div>
                                        </button>
                                    );
                                }) : <p className="text-zinc-500 text-center col-span-full py-10">Este é um dia de folga ou ausência.</p>}
                            </div>
                        )}
                    </div>
                </main>
