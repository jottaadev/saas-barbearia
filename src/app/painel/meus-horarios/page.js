...
export default function MeusHorariosPage() {
    ...
    const fetchDaySchedule = useCallback(async (date, token) => {
        ...
        try {
            ...
            const response = await axios.get(`http://localhost:3333/api/barber/schedule?date=${formattedDate}`, {
    ...
    const fetchAbsences = useCallback(async (token) => {
        ...
        try {
            const response = await axios.get('http://localhost:3333/api/barber/absences', { headers: { 'Authorization': `Bearer ${token}` } });
