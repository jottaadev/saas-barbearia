...
import { AlertCircle, TrendingUp, Users, RefreshCw, Calendar as CalendarIcon, BarChartHorizontalBig } from 'lucide-react';
...
// --- Componente Card para os Gráficos ---
        ...
        const fetchInitialData = async () => {
            ...
            try {
                const [performanceResponse, barbersResponse] = await Promise.all([
                    axios.get(getStrapiURL('/api/admin/reports/performance'), { headers: { 'Authorization': `Bearer ${token}` } }),
                    axios.get(getStrapiURL('/api/users/profiles'), { headers: { 'Authorization': `Bearer ${token}` } })
                ]);
                setReportData(performanceResponse.data);
                setAllBarbers(barbersResponse.data.filter(b => b.role === 'barber'));
            } catch (err) {
    ...
    useEffect(() => {
        ...
        const fetchHistory = async () => {
            ...
            try {
                ...
                const response = await axios.get(getStrapiURL('/api/admin/appointments'), { headers: { 'Authorization': `Bearer ${token}` }, params });
    ...
    return (
        <div className="bg-zinc-950 min-h-screen text-white flex font-sans">
            ...
            <main className="flex-1 p-6 sm:p-8 overflow-y-auto">
                <h1 className="font-display font-bold text-4xl text-white mb-8 flex items-center gap-3">
                    <BarChartHorizontalBig className="text-amber-500"/>
                    Relatórios
                </h1>
                    ...
                    <ReportCard title="Faturação por Barbeiro" icon={TrendingUp} isLoading={isLoading.reports}>
                        <ResponsiveContainer>
                            <BarChart data={reportData?.barberPerformance} layout="vertical" margin={{ top: 5, right: 30, left: 10, bottom: 5 }}>
                                ...
                                <Bar dataKey="total_revenue" name="Faturação" fill="#f59e0b" radius={[0, 4, 4, 0]} />
                            </BarChart>
                        </ResponsiveContainer>
                    </ReportCard>
                    ...
                    <ReportCard title="Atendimentos por Barbeiro" icon={Users} isLoading={isLoading.reports}>
                        <ResponsiveContainer>
                            <BarChart data={reportData?.barberPerformance} layout="vertical" margin={{ top: 5, right: 30, left: 10, bottom: 5 }}>
                                ...
                                <Bar dataKey="completed_appointments" name="Atendimentos" fill="#3b82f6" radius={[0, 4, 4, 0]} />
                            </BarChart>
                        </ResponsiveContainer>
                    </ReportCard>
                </div>
                    ...
                    <div className="bg-zinc-900 rounded-lg border border-zinc-800 overflow-hidden">
                        <div className="max-h-[50vh] overflow-y-auto">
                            <table className="w-full text-left">
                                ...
                                <tbody>
                                    {isLoading.history ? (
                                        <tr><td colSpan="4" className="p-8 text-center text-zinc-500">A carregar histórico...</td></tr>
                                        ...
                                        <tr><td colSpan="4" className="p-8 text-center text-zinc-500">Nenhum agendamento encontrado para os filtros selecionados.</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
