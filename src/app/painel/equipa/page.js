...
const BarberModal = ({ barber, onClose, onSave, error }) => {
  ...
  const previewUrl = avatarFile ? URL.createObjectURL(avatarFile) : (formData.avatar_url ? `http://localhost:3333${formData.avatar_url}` : null);
...
export default function GerirEquipaPage() {
  ...
  const fetchTeam = async () => {
    ...
    try {
      ...
      const response = await axios.get('http://localhost:3333/api/users/profiles', { headers: { 'Authorization': `Bearer ${token}` } });
  ...
  const handleSaveBarber = async (formData, isEditing) => {
    ...
    const url = isEditing ? `http://localhost:3333/api/users/${editingBarber.id}` : 'http://localhost:3333/api/users';
  ...
  const handleDeleteBarber = async (barberId) => {
    if (window.confirm('Tem a certeza que quer apagar este membro da equipe?')) {
      const token = sessionStorage.getItem('authToken');
      try {
        await axios.delete(`http://localhost:3333/api/users/${barberId}`, { headers: { 'Authorization': `Bearer ${token}` } });
        fetchTeam();
      } catch (err) {
              ...
              {team.map(barber => (
                <tr key={barber.id} className="border-t border-zinc-800 hover:bg-zinc-800/50">
                  <td className="p-4 font-medium text-white flex items-center gap-3">
                    <img src={barber.avatar_url ? `http://localhost:3333${barber.avatar_url}` : '/default-avatar.png'} alt={barber.name} className="rounded-full object-cover w-10 h-10"/>
                    {barber.name}
                  </td>
