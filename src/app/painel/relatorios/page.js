            ...
            const fetchInitialData = async () => {
                ...
                try {
                    const [performanceResponse, barbersResponse] = await Promise.all([
                        axios.get('http://localhost:3333/api/admin/reports/performance', { headers: { 'Authorization': `Bearer ${token}` } }),
                        axios.get('http://localhost:3333/api/users/profiles', { headers: { 'Authorization': `Bearer ${token}` } })
                    ]);
    ...
    useEffect(() => {
        ...
        const fetchHistory = async () => {
            ...
            try {
                ...
                const response = await axios.get('http://localhost:3333/api/admin/appointments', {
